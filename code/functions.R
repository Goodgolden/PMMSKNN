## functions.R ---------------------------------------------
# Mon Apr 05 10:20:36 2021 ------------------------------

##  preprocess ---------------------------------------------
##  create train/test filtering
preproc <-
  function(dff = full,
           ...
           # select knots that are clinically relevat
           out_time = 90,
           # this variable has to be within the knots above
           outcome = "tug",
           time_var = "time",
           pat_id = "patient_id",
           # Need the patient_id column
           # to be specified as "patient_id"
           baseline_var = "baseline",
           # pre-op/post-op indicator variable.
           # preop: baseline = 1;
           # postop: baseline = 0 or otherwise
           varlist = NULL,
           # need user to fill in var list
           # otherwise will use all vars,
           # categorical variables need
           # to be factor or character
           pmmform = NULL,
           modelselect = FALSE,
           ...) {
    ...
    if (is.null(baseline_var)) {
      stop("baseline_var is NULL.
            ...
            Utility function baselinemk() may be used to
            create baseline variable.")
    }

    ## baseline okay ---------------------------------------------
    # If varlist supplied, then form dataframe that only contains
    # ID, Outcome, Time, train/test split variable, 
    # and Listed variables else stop function
    if (!is.null(varlist)) {
      dff <- dff %>%
    ...
    ## split test traning ---------------------------------------------
    # Mon Apr 05 10:28:23 2021 ------------------------------
    # Split test/train by pre and post
    # change the filter_() into filter()

    df_train <- dff %>% 
        filter(paste0(split_var, "==", trainval))
    df_test <- dff %>% 
        filter(paste0(split_var, "==", testval))
...
loocv_function <- function(nearest_n = seq(20, 150, by = 10), # number to play with
  ...
  if (is.null(dist_fam)) {
    ...
    ref <- list(ref1, ref2, ref3, ref4, ref5)[which.min(lapply(list(ref1, ref2, ref3, ref4, ref5), function(x) {
      x$aic
    }))][[1]]
  ...
  if (is.null(time_window)) {
    ...
    iqrfull <- centiles.pred(ref,
      type = "centiles", xname = time_elapsed, xvalues = c(mint:maxt),
      data = train_post,
      cent = c(25, 75), plot = FALSE
    )
  } else {
    iqrfull <- centiles.pred(ref,
      type = "centiles", xname = time_elapsed, xvalues = c(time_window[1]:time_window[2]),
      data = train_post,
      cent = c(25, 75), plot = FALSE
    )
  }
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  temp <- data.frame(
    train_id = sapply(1:length(test_o$pred), function(x) {
    ...
    for (n in nearest) {
      ...
      for (i in patlistint) {
        ...
        if (matchprobweight) {
          matchprob <- ord_data %>%
            ...
            .[-1, ] %>%
            head(n = n) %>%
            cbind(weight = seq(n, 1) / sum(seq(n, 1))) %>%
            as.data.frame() %>%
            rename(patient_id = 1)
      ...
      if (loocv != TRUE) {
        ...
        if (plot == TRUE) {
          ...
          df <- data.table(
            mrmse = sapply(nn_arr, function(x) {
...
plot_cal <- function(plotobj,
  ...
  plot_func <- function(plotobj = plotobj, train = TRUE, filt = filt) {
    ...
    if (train == TRUE) {
      ...
      if (obs_dist == "gaussian" | obs_dist == "poisson") {
        if (obs_dist == "gaussian") {
          cptrain <- ggplot(plotdf, aes(x = avg_val, y = estimate, ymin = estimate - 1.96 * std.error, ymax = estimate + 1.96 * std.error)) +
            geom_pointrange() +
          ...
          cptrain <- ggplot(plotdf, aes(x = avg_val, y = estimate, ymin = ll, ymax = ul)) +
            geom_pointrange() +
            # xlim(minc, maxc) + ylim(minc,maxc) +
            geom_abline(slope = 1, intercept = 0) +
            xlab(paste0("Predicted ", toupper(outcome))) +
      ...
      cptrainzsc <- ggplot(plotdf, aes_string(
        x = "avg_val",
        y = "zsc",
        group = "avg_val"
      )) +
      ...
      if (obs_dist == "gaussian" | obs_dist == "poisson") {
        if (obs_dist == "gaussian") {
          cptest <- ggplot(plotdf, aes(x = avg_val, y = estimate, ymin = estimate - 1.96 * std.error, ymax = estimate + 1.96 * std.error)) +
            geom_pointrange() +
            # xlim(minc, maxc) + ylim(minc,maxc) +
            geom_abline(slope = 1, intercept = 0) +
            xlab(paste0("Predicted ", toupper(outcome))) +
          ...
          cptest <- ggplot(plotdf, aes(x = avg_val, y = estimate, ymin = ll, ymax = ul)) +
            geom_pointrange() +
            # xlim(minc, maxc) + ylim(minc,maxc) +
            geom_abline(slope = 1, intercept = 0) +
            xlab(paste0("Predicted ", toupper(outcome))) +
      ...
      cptestzsc <- ggplot(plotdf, aes_string(
        x = "avg_val",
        y = "zsc",
        group = "avg_val"
      )) +
...
loocvperf <- function(loocv_res) {
  ...
  data.frame(
    ...
    mutate(
      nearest_n = as.numeric(gsub("nearest_", "", nearest_n)),
...
extvalid <- function(loocvobj,
  ...
  temp <- data.frame(
    train_id = sapply(1:length(test_proc$test_o$pred), function(x) {
      # which.min(abs(test_proc$test_o$pred[x] - test_proc$train_o$Fitted))
  ...
  res <- data.frame(
    zscore = mean(rbindlist(loocvobj$pred_res$zsc_list)$zsc[!is.na(rbindlist(loocvobj$pred_res$zsc_list)$zsc) & !is.infinite(rbindlist(loocvobj$pred_res$zsc_list)$zsc)]),
