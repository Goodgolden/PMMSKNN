  ...
  if (train) {
    ...
    perfdf <- rbindlist(perflist) %>%
      ...
      mutate(
        ...
        covsc = if ((max(.data$covdiff, na.rm = TRUE) - min(.data$covdiff, na.rm = TRUE)) == 0) {
          return(0)
        } else {
          (.data$covdiff - min(.data$covdiff, na.rm = TRUE)) / (max(.data$covdiff, na.rm = TRUE) - min(.data$covdiff, na.rm = TRUE))
        },
