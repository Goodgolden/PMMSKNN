...
plot_func <- function(plotobj = plotobj,
  ...
  if (train == TRUE) {
    ...
    if (is.null(names(plotobj$loocv_res))) {
      nearest_n_idx <- which({
        unlist(lapply(plotobj$loocv_res, function(x) {
    ...
    if (obs_dist == "gaussian" | obs_dist == "poisson") {
      ...
      if (obs_dist == "gaussian") {
        ...
        plotdf <- pred_tug %>%
          ...
          mutate(
            ul = .data$estimate + 1.96 * .data$std.error,
    ...
    if (obs_dist == "gaussian" | obs_dist == "poisson") {
      if (obs_dist == "gaussian") {
        cptrain <- ggplot(plotdf, aes(
          x = .data$avg_val, y = .data$estimate,
    ...
    if (!is.null(iqrfull)) {
      if (any(colnames(iqrfull) %in% "C50")) {
        ...
        cptrainrefdf <- test_proc$train_post %>%
          ...
          left_join(
            test_proc$train_post %>%
              ...
              mutate(dec = ntile(.data$C50, 10)) %>% left_join(
                test_proc$train_post %>%
                  dplyr::select_("patient_id", outcome, "time") %>%
                  left_join(
                    iqrfull,
        ...
        return(list(cptrain + theme_bw() + geom_pointrange(aes(
          x = cptrainrefdf$avg_val,
