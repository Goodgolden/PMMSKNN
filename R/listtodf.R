...
#' tables and plots
#'
#' @param file - The leave-one-out-cross-validation result list
#' component created using the \code{loocv_function}.
#' If \code{x <- loocv_function()}, then \code{x$loocv_res} would be
#' the \code{file} to be inputted into the function.
#'
#' @return A data frame that contains performance values 
#'         (bias, coverage probabilities, zscore, rmse, and dropped cases) 
#'         by the number of matches (per row)
listtodf <- function(file) {
  temp <- lapply(file, 
                 function(x) {
                   c(mean(x$bias, na.rm = TRUE),
                     mean(x$iqrcoverage, na.rm = TRUE),
                     mean(x$coverage95c, na.rm = TRUE),
                     mean(rbindlist(x$zscore)$zsc[!is.na(rbindlist(x$zscore)$zsc) & 
                                                    !is.infinite(rbindlist(x$zscore)$zsc)]),
  ...
  colnames(temp) <- regmatches(names(file), 
                               regexpr("\\d+", names(file)))
  rownames(temp) <- paste0(c("bias", "iqrcoverage", 
                             "coverage95c", 
                             "zscore", "iqrdif", 
                             "rmse", "dropped_cases"))
  temp$measure <- c("bias", "iqrcoverage", 
                    "coverage95c", "zscore", 
                    "iqrdif", "rmse", "dropped_cases")
